...
const Description = () => {
    ...
    useEffect(() => {
        ...
        const checkUserEnrollment = async () => {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/api/payments/check-enrollment/${courseId}`, {
                    headers: {
    ...
    if (loading || checkingEnrollment) return <div className="text-white text-center mt-20">Loading course details...</div>;
    ...
    return (
        <div className="">
            ...
            <div className="py-8 md:py-12">
                <div className="mt-24 md:mt-32 px-6 md:px-12 lg:px-28 text-center flex flex-col gap-6 md:gap-9">
                    <h1 className="text-white font-corinthia text-4xl md:text-6xl lg:text-8xl font-bold tracking-tight bg-gradient-to-r from-white via-orange-100 to-white bg-clip-text text-transparent">
                        {courseData?.title || "Welcome to the Course"}
                    </h1>
                    <img src={fallbackCohortSvg} alt="cohort" className="w-full md:w-[70%] max-w-4xl mx-auto opacity-90 transition-opacity hover:opacity-100"/>
                    <p className="text-neutral-300 text-lg md:text-3xl font-light leading-relaxed max-w-4xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-7">
                    <div 
                        ...
                        className={`cursor-pointer rounded-tr-lg rounded-bl-lg flex items-center px-4 hover:scale-105 transition-transform ${isEnrolled ? 'bg-gradient-to-r from-orange-600 to-amber-500 text-white shadow-lg shadow-orange-950/20' : 'bg-white text-black'} ${enrolling ? 'opacity-50 pointer-events-none' : ''}`}
                    >
                        <Button className={`text-sm font-corinthia ${isEnrolled ? 'text-white' : 'text-black'}`}>
                            {enrolling ? "Redirecting..." : isEnrolled ? "Start Learning" : "Enroll Now"}
                        </Button>
                        <GoArrowUpRight className={isEnrolled ? 'text-white' : 'text-black'} />
                    </div>
                    <div 
                        onClick={() => {
                            const element = document.getElementById("syllabus-section");
                        ...
                        className="cursor-pointer flex items-center bg-black border border-neutral-800 rounded-tr-lg rounded-bl-lg px-4 hover:bg-neutral-900 transition-colors"
                    >
            ...
            {/* Video */}
            <div className="px-6 md:px-12 lg:px-28 my-10 md:my-20">
                <iframe 
                    ...
                    className="mx-auto w-full max-w-4xl aspect-video rounded-2xl border border-neutral-800 shadow-2xl"
                    frameborder="0"
            ...
            {/* What you will learn */}
            <div id="syllabus-section" className="px-6 md:px-12 lg:px-28 scroll-mt-24">
                <p className="text-2xl md:text-4xl text-white text-center mb-6">What you'll <span className="text-orange-100">Learn</span></p>
                <p className="text-lg md:text-xl text-neutral-300 text-center">Six months of a <span className="font-bold">Structured</span>, hands-on curriculum</p>
                <p className="text-lg md:text-xl text-neutral-300 text-center">designed to take you from <span className="font-bold">Fundaments</span> to Advanced concepts.</p>
                <div className="flex justify-end gap-2 py-6 md:py-10">
            ...
            {/* Syllabus Data */}
            <div className="px-6 md:px-12 lg:px-28 pb-20">
                        ...
                        {openSections[section.id] && (
                            <div className="bg-neutral-950 p-4 border-t border-neutral-800">
                                {section.lessons?.map((lesson) => (
                                    <div key={lesson.id} className="py-3 px-4 text-neutral-400 border-b last:border-0 border-neutral-900 flex items-center justify-between group hover:bg-white/5 transition-colors rounded-lg">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={isEnrolled ? "text-neutral-200" : "text-neutral-400"}>{lesson.title}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {isEnrolled ? (
                                                ...
                                                <div className="flex items-center gap-1.5 text-xs text-neutral-500 bg-neutral-800 px-2.5 py-1 rounded-full border border-neutral-700/50">
                                                    <HiLockClosed className="text-sm text-orange-500" />
                                                    <span>Locked</span>
                                                </div>
                                            )}
                                        </div>
